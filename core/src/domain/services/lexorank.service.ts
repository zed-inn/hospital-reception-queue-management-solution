export class LexoRank {
  static readonly min = "000000";
  static readonly max = "zzzzzz";
  ...
  static add(a: string, val: number) {
    const newVal = this.getNumericValue(a) + BigInt(Math.ceil(val));
    return this.createBaseString(newVal);
  }

  static subtract(a: string, val: number) {
    const newVal = this.getNumericValue(a) - BigInt(Math.ceil(val));
    return this.createBaseString(newVal);
  }
